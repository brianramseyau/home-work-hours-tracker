<script lang="ts">
	...
	// the cog in its header goes to the years CRUD page. The year it shows is the one being
	// viewed, falling back to the current open period on non-diary routes.
	let {
		fy,
		years,
		compact = false
	}: { fy: FySummary; years: FySummary[]; compact?: boolean } = $props();
				...
				{#each years as year (year.startYear)}
					<li>
						<a
							href={`/${year.slug}`}
							onclick={() => (open = false)}
							aria-current={year.startYear === fy.startYear ? 'page' : undefined}
							class={cn(
								'flex items-baseline justify-between gap-3 rounded-md px-2.5 py-1.5 hover:bg-accent',
								year.startYear === fy.startYear && 'bg-accent'
							)}
						>

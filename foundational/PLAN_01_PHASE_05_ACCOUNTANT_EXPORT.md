...
- **Diary row Hours are not rounded per row** (`=IF(F="","",(G-F)*24-H/60)`, no `ROUND`) — an earlier cut rounded each row to 2 dp, which for non-round block times (e.g. two 457-minute blocks) made the sheet's own total and claim drift by up to a cent from `core/totals.claimCents`. The total and annual claim are computed in integer minutes alongside the rows themselves (`buildDiaryRows` returns `totalMinutes`/`minutesByMonth` derived from exactly the blocks it emits, not from a separate, potentially-wider pass over `input.days`), then converted via `claimCents()`; the twelve monthly cached claims use `claimCentsByGroup` so they always sum to the annual figure (the live per-cell `ROUND` formula can still independently round a given month by up to a cent on recalculation — an inherent spreadsheet-formula limitation, not a bug, and the same tension `MonthBreakdown.svelte`'s own comment already documents for the in-app view). The Diary sheet's Hours column (both the rows and its own totals cell) shows **4 decimal places**, not the app's usual 2 — at 2 dp the unrounded row values can visibly fail to sum to the printed total (e.g. 7.62 + 7.62 printing next to a 15.23 total), which would read as an error to the accountant this sheet is for; the Summary sheet's figures stay at 2 dp / currency formatting, since those are the practical final numbers, not an audit ledger.

...
- **Generated files** (`scripts/generate-icons.mjs`): `favicon.svg` (dark-scheme aware), `favicon.ico` (16/32/48), `apple-touch-icon.png` (180), PWA `icon-{192,512}.png` plus maskable variants (ink ground, safe-zone padding), `src/lib/assets/logo-mark-light.png` (for the xlsx — not `logo-lockup.png`, which was never built; see the P05 decisions-log entry), and `docs/social-preview.png` (1280×640, not yet built).
...
| 2026-09-14 | P04   | The Diary's mobile week pager and desktop table are genuinely separate components (`WeekPager`/`DiaryTable`), not one component with responsive CSS — a table with sticky headers and inline time editing doesn't collapse cleanly into a swipeable day list. Which one renders (and whether the day editor opens as a `Drawer` or `Sheet`) is decided in script by `svelte/reactivity/window`'s `innerWidth`, at the same 1024px breakpoint the CSS itself already uses via `lg:`.                                                                                                                                                                                                                                                                                                                                                                                                     |
| 2026-09-14 | P04   | The punch card's per-cell tooltip uses a native `title` attribute plus an `aria-label`, not a styled `Tooltip` component — 365+ cells is a lot of overlay instances for a glance-only hover detail, and axe was clean either way.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| 2026-09-14 | P04   | A day's kind is edited as a 7-way segmented control (Home, Office, Split, Leave, Sick, Holiday, Off) rather than the underlying 5-value `DayKind` — Home/Office/Split all persist as `kind: 'work'`, distinguished by whether the day has blocks, an office, or both. The control speaks the same language as the punch card's legend, not the schema.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| 2026-09-14 | P05   | The outlined wordmark lockup this log's own P01 entry deferred to P05 was **not** created — the export's Summary title band uses the existing logo mark on its own, next to the live Bricolage Grotesque title text, rather than blocking the exporter on new SVG/PNG asset work. `logo-lockup.svg`/`logo-lockup.png` still don't exist; a future pass (e.g. bundled with P07's icon/branding work) should either produce them or drop the references to them from this file's Assets/Generated-files lists. The mark PNG itself moved from `static/brand/logo-mark-light.png` to `src/lib/assets/logo-mark-light.png` (`scripts/generate-icons.mjs` now writes it there) so the exporter can bundle it at build time via a Vite `?inline` import instead of reading a `static/`-relative path at server runtime, which isn't guaranteed to resolve once deployed under `adapter-node`. |
| 2026-09-14 | P06   | The import review's proposed-office picker (Create new / Map to an existing office / Ignore, per proposed name) uses `ToggleGroup`, the same reasoning as P03's schedule-editor office picker — a dynamic, data-driven list of options with no hidden `?? ''` branch. The per-row "include in this import" checkboxes are plain native `<input type="checkbox">` with an `aria-label` (`Import 2026-07-06`), not a styled control — a two-hundred-row table is a lot of component instances for a binary include/exclude toggle, and a native checkbox is unambiguous there.                                                                                                                                                                                                                                                                                                            |
| 2026-09-15 | Fix   | Every future day with no row now previews the schedule as a ghost row, not only the rest of the current week — otherwise the rest of the year read as a run of "Off" days that simply hadn't happened yet. Ghost rows are muted and keep the "From schedule" marker, and every day after today stays hatched in the rows, the punch card and the strip alike (a preview is a plan, not a record); a preview cell's label omits hours. In the Diary and punch card a future day the schedule doesn't cover reads "Not yet", not "Off"; a future day's editor opens on what the schedule will make it (Off, for an unscheduled one). A finalised year previews nothing, since nothing is ever planned into it.                                                                                                                                                                            |
| 2026-09-15 | Fix   | The office a day was worked at is shown in the Diary itself: a quiet column in the desktop table and a muted second line under the type in the mobile day row. Archived offices still resolve their name (the list already includes them). The Years page is inverted — newest first, with "Add FYnn" moved up beside the heading — and the shell's FY badge becomes the switch this file's P03 entry promised: a pop-out (shadcn `Popover`) listing every year newest-first for direct "open diary" switching, with a gear in its header for the years CRUD page. The badge shows the year being viewed (`fyFromPath` off the pathname), falling back to the current open period on non-diary routes; `/` still always opens the current period. The pop-out carries `role="region"` so its portalled content stays inside a landmark for axe.                                         |

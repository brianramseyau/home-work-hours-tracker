...
- **The only coverage exclusions:** `src/lib/components/ui/**` (vendored shadcn-svelte), `**/*.d.ts`, `src/app.html`, `drizzle/**`, `scripts/**` except `scripts/pii/**`, config files, and the `@vite-pwa` service-worker glue. Any addition needs a justification in the phase doc and must be listed here.
